    ...
    <table class="border-separate border-gray-500 -mt-10px">
      <thead>
        <tr>
          <th class="w-1/5 px-4 py-2">
            <input type="checkbox" value="all" class="float-left h-5 w-5" />
          </th>
          <th class="w-1/5 px-4 py-2">
            EMPLOYEE
      ...
      <tbody>
        <tr class="bg-white" v-for="users in allUsers" :key="users.id">
          <td class="px-4 py-2">
            <input type="checkbox" :value="users.id" class="h-5 w-5 mt-3" />
            <img
          ...
          <td class="px-4 py-2">
            ...
            <p class="font-bold text-gray-500">{{ users.employee_role }}</p>
          </td>
          <td class="px-4 py-2">
            <p class="font-bold">{{ users.employee_salary }}</p>
            <p class="font-bold text-gray-500">{{ users.employment_type }}</p>
          </td>
          <td class="px-4 py-2">
            <p class="font-bold">{{ users.employment_status }}</p>
            <p class="font-bold text-gray-500">{{ users.created_at }}</p>
          </td>
          <td class="px-4 py-2 text-2xl text-gray-500">
            <span class="mx-5 mt-4 cursor-pointer" @click="getOneUser(users.id)">
              <i class="fas fa-pencil-alt"></i>
            </span>
            <span class="mt-4 cursor-pointer" @click="deleteUser(users.id)">
